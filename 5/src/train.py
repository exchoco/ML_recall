...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor(self.data.iloc[idx, self.data.columns.get_loc('gt_label')], dtype=torch.long) # changed here
        return image, label
...
def train_model(train_loader, val_loader, num_epochs=10, num_classes=10): #change here
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Load pre-trained ResNet-50 model
    model = resnet50(pretrained=True)
    num_ftrs = model.fc.in_features
    model.fc = nn.Linear(num_ftrs, num_classes)  # Binary classification output #change here
    model = model.to(device)

    # Loss function and optimizer
    criterion = nn.CrossEntropyLoss() # change here
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_loader:
            ...
            _, predicted = torch.max(outputs, 1) #change here
...
def evaluate_model(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in data_loader:
            images, labels = images.to(device), labels.to(device)
            # labels = labels.unsqueeze(1)  # Ensure labels have the correct shape #change here
            outputs = model(images)
            loss = criterion(outputs, labels)
            val_loss += loss.item()
...
def infer_and_save_results(model, test_loader, output_csv):
    ...
    with torch.no_grad():
        for i, images in enumerate(test_loader):
            ...
            outputs = model(images)
            _, predicted = torch.max(outputs, 1) #change here
            results.extend(predicted.cpu().numpy()) #changed here
...
def main():
    ...
    model = train_model(train_loader, val_loader, num_epochs=30, num_classes=10) # change here
