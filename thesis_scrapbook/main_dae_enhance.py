...
from model_dae_enhance import VAE_withCrossSkip128skipMiddleLossDisent_Unroll, TripletNet, VAE_withCrossSkip128skipMiddleLoss_Unroll, VAE_withCrossSkip128skipMiddleLossConv_Unroll, VAE_withCrossSkipMiddleLossConv_Unroll
from utils import *
import pytorch_ssim
...
if __name__=='__main__':
    ...
    # Announce the model
    start_epoch = 0
    net = VAE_withCrossSkipMiddleLossConv__Disent_Unroll()
    siam_net = TripletNet(net)
    siam_net = nn.DataParallel(siam_net, device_ids=[0,1,3])

    # siam_net.load_state_dict(torch.load("./mod_weight/TriVAE480.pt"))
    # start_epoch = 480
    siam_net.to(device)
    ...
    for epoch in range(start_epoch, start_epoch + args.epoch + 1):
        ...
        for idx, x in enumerate(batch):
            ...
            enc_triplet_loss = triplet_criterion(out_anchor[3], out_pos[3], out_neg[3])
        ...
        if(int(epoch) % args.save_weight_interval == 0):
            ...
            if(int(epoch) >= 300 or int(epoch) == 0):
