...
if __name__=='__main__':
    ...
    disc_model.load_state_dict(torch.load("./asc_step1_weight/Discriminator2500.pt"))
    # start_epoch = 10
            ...
            for j, (x_tumor) in enumerate(eval_dataset_loader):
                ...
                for idx in range(int(math.floor(x_tumor["image"].shape[0] / args.batch_size))):
                    ...
                    for _ in range(args.n_gen):
                        ...
                        img_tumor_double = torch.cat((img_tumor, img_tumor), 0)
                        fake_wild, fake_fence, fake_combined = gen_model(img_tumor_double)
                        # Real images
                        predicted_validity = disc_model(fake_fence)
                        target_validity = torch.ones(fake_fence.shape[0], 1) * -1
                        ...
                        mse_recons = mse_criterion(fake_combined, img_tumor_double)
                        disjoincy = special_loss_disjoint_step2(fake_wild, fake_fence)

                        # g_loss = 0.5 * mae_validity + 0.1 * mse_recons + 0.4 * disjoincy
                        g_loss = mae_validity + mse_recons + disjoincy * 100
                    ...
                    for _ in range(args.n_critic):
                        ...
                        # instead of directly like step 1, now add the healthy from generated fence cut
                        real_wild, real_fence, real_combined = gen_model(img_healthy)
                        img_healthy_fhealthy = torch.cat((real_fence, img_healthy), 0)
                        healthy_labels_double = torch.cat((healthy_labels, healthy_labels), 0)

                        # adjust fake batch size Generate a batch of fake images from tumor imgs
                        img_tumor_double = torch.cat((img_tumor, img_tumor), 0)
                        fake_wild, fake_fence, fake_combined = gen_model(img_tumor_double)
                        fake_labels = torch.ones(img_tumor_double.shape[0], 1).to(device)

                        # combine the fake fence and healthy img and label
                        combined_imgs = torch.cat((img_healthy_fhealthy, fake_fence), 0)
                        combined_labels = torch.cat((healthy_labels_double, fake_labels), 0)
