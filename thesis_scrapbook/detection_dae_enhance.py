...
if __name__=='__main__':
    ...
    # siam_net.load_state_dict(torch.load("./tvae_c_complete/mod_weight/TriVAE1100.pt"))
    siam_net.load_state_dict(torch.load("./mod_weight/TriVAE1000.pt"))
    siam_net.to(device)
            ...
            for subj_idx, x in enumerate(eval_dataset_loader):
                ...
                for idx in range(int(math.ceil(original_image.shape[0] / args.batch_size))):
                    if(original_image.shape[0] - 1 >= ((idx + 1) * args.batch_size) ):
                        ...
                        current_slice = start_idx
                    else:
                        remaining = original_image.shape[0] % args.batch_size
                        start_idx = -remaining
                        end_idx = None
                        current_slice = original_image.shape[0] - remaining - 1
                    fname = x["filenames"]
                    print(f"this is file names {fname}")
                    print(f"current idx 0 is slice {current_slice}")
                    ...
                    #     plt.grid(False)
                    #     plt.imshow(gridify_output(out, 1), cmap='gray')
    ...
    tsne = TSNE(n_components=2, init='random', random_state=0, verbose=0, perplexity=30).fit_transform(features.cpu())
    tx = tsne[:, 0]
    ty = tsne[:, 1]
    # tz = tsne[:, 2]
