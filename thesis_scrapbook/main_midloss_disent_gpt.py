...
from model import (CNN_Encoder, 
    ...
    CNN_Decoder_SpatialCrossSkip3OnlyMiddleLossWith128Skip_disent,
    CNN_EncoderWithCoorAvgPool_disent,
...
if __name__=='__main__':
    ...
    parser.add_argument("-save_weight_interval" , type = int , default = 30)
    parser.add_argument("-batch_size" , type = int , default = 28)
    parser.add_argument("-Learning_rate_enc" , type = float , default = 1e-3)
    parser.add_argument("-Learning_rate_encdec" , type = float , default = 1e-3)
    ...
    # Announce the model
    start_epoch = 0
    Encoder = CNN_EncoderWithCoorAvgPool_disent()
    Decoder = CNN_Decoder_SpatialCrossSkip3OnlyMiddleLossWith128Skip_disent()
    ...
    # siam_enc.load_state_dict(torch.load("./mod_weight/TriEncoder540.pt"))
    # siam_dec.load_state_dict(torch.load("./mod_weight/TriDecoder540.pt"))
    # start_epoch = 540
    siam_enc.to(device)
    siam_dec.to(device)

    params =  list(siam_enc.parameters()) + list(siam_dec.parameters())
    enc_dec_optimizer = torch.optim.Adam(params, lr=args.Learning_rate_enc, weight_decay=args.weight_decay)
    ...
    # enc_dec_scheduler = lr_scheduler.ReduceLROnPlateau(enc_dec_optimizer, 'min')

    enc_optimizer = torch.optim.Adam(siam_enc.parameters(), lr=args.Learning_rate_encdec, weight_decay=args.weight_decay)
    ...
    # enc_scheduler = lr_scheduler.ReduceLROnPlateau(enc_optimizer, 'min')
    ...
    # this is a new triplet function
    ...
    for epoch in range(start_epoch, start_epoch + args.epoch + 1):
        ...
        for idx, x in enumerate(batch):
            ...
            # x["less_noisy_positive"] = add_coarse_noise_lesser(x["positive"].clone())

            out_anchor, out_pos, out_neg = siam_enc(x["negative_ori"], x["positive"], x["noisy_anchor"])
            ...
            # train both encoder decoder for the reconstruction
                ...
                rec_pos_64 = rec_pos[1]
                rec_neg_64 = rec_neg[1]
                ...
                TV_anchor = TV_criterion(torch.abs(torch.sub(rec_anchor, x["negative_ori"])))
                TV_pos = TV_criterion(torch.abs(torch.sub(rec_pos, x["positive"])))
                TV_neg = TV_criterion(torch.abs(torch.sub(rec_neg, x["anchor"])))
                ...
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + mae_neg_to_ori) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                
                #the best
                # enc_dec_loss = 5000 * (0.1 * mae_anchor + 0.1 * mae_pos + 0.8 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                #A_1 without the SSIM loss
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + 20 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos)
                #A_2 without the middle loss and A_4 without skip only Tri-VAE (use the main.py)
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + 20 * mae_neg_to_ori) + (kld_anchor + kld_pos)
                #A_3_SSIM this is for comparison for complete version
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + 20 * mae_neg_to_ori) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)

                # try adding negative kld
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + 20 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos + kld_neg) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                # enc_dec_loss = 5000 * (0.05 * mae_anchor + 0.05 * mae_pos + 0.9 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos + kld_neg) - (SSIM_anchor + SSIM_pos + SSIM_neg)

                # bard loss no healhy reconstruction
                enc_dec_loss = (mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos) - (SSIM_neg)

                
                # enc_dec_loss = 5000 * (MMSE_anchor + MMSE_pos + MMSE_neg + MMSE_anchor_64 + MMSE_pos_64 + MMSE_neg_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                ...
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64 - mae_neg_to_noisy_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + mae_neg_to_ori - mae_neg_to_noisy + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64 - mae_neg_to_noisy_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
        ...
        # enc_dec_scheduler.step(enc_dec_loss)
        # enc_scheduler.step(enc_triplet_loss)
        ...
        if(int(epoch) % args.save_weight_interval == 0):
            ...
            if(int(epoch) >= 250 or int(epoch) == 0):
