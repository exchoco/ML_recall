...
if __name__=='__main__':
    ...
    siam_enc.to(device)
    siam_enc.load_state_dict(torch.load("./mod_weight_best/TriEncoder580.pt"))
    siam_dec.to(device)
    siam_dec.load_state_dict(torch.load("./mod_weight_best/TriDecoder580.pt"))
                    ...
                    if(args.vae_restoration):
                        with torch.enable_grad():
                            ...
                            for step in range(args.restoration_steps):
                                ...
                                SSIM_loss.cuda()
                                out_anchor = siam_enc.module.get_embedding(x_pred)
