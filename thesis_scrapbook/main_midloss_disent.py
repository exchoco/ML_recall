...
from model import (CNN_Encoder, 
    ...
    CNN_Decoder_SpatialCrossSkip3OnlyMiddleLossWith128Skip_disent_gnorm,
    CNN_EncoderWithCoorAvgPool_disent_gnorm,
    CNN_Decoder_SpatialCrossSkip3OnlyMiddleLossWith128Skip_disent_nosigmoid)
from utils import *
import pytorch_ssim
...
if __name__=='__main__':
    ...
    Decoder = CNN_Decoder_SpatialCrossSkip3OnlyMiddleLossWith128Skip_disent_nosigmoid()
    ...
    # siam_enc.load_state_dict(torch.load("./mod_weight/TriEncoder280.pt"))
    # siam_dec.load_state_dict(torch.load("./mod_weight/TriDecoder280.pt"))
    # start_epoch = 340
    siam_enc.to(device)
    siam_dec.to(device)
    ...
    for epoch in range(start_epoch, start_epoch + args.epoch + 1):
        ...
        for idx, x in enumerate(batch):
            ...
            # train both encoder decoder for the reconstruction
                ...
                #the best
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + 20 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                ...
                # enc_dec_loss = 5000 * (MMSE_anchor + MMSE_pos + MMSE_neg + MMSE_anchor_64 + MMSE_pos_64 + MMSE_neg_64) + (kld_neg) - (SSIM_anchor + SSIM_pos + SSIM_neg)
                # enc_dec_loss = 5000 * (mae_anchor + mae_pos + mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_neg) - (SSIM_anchor + SSIM_pos + SSIM_neg)

                enc_dec_loss = (0.15 * mae_anchor + 0.15 * mae_pos + 0.7 * mae_neg_to_ori + mae_anchor_64 + mae_pos_64 + mae_neg_to_ori_64) + (kld_anchor + kld_pos) - (SSIM_anchor + SSIM_pos + SSIM_neg)
