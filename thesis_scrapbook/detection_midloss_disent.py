...
if __name__=='__main__':
    ...
    siam_enc.to(device)
    siam_enc.load_state_dict(torch.load("./mod_weight/TriEncoder510.pt"))
    siam_dec.to(device)
    siam_dec.load_state_dict(torch.load("./mod_weight/TriDecoder510.pt"))
            ...
            for subj_idx, x in enumerate(eval_dataset_loader):
                ...
                for idx in range(int(math.ceil(original_image.shape[0] / args.batch_size))):
                    ...
                    if features is not None:
                        features = torch.cat((features, out_anchor[4].view(-1, out_anchor[4].shape[-1])))
                    else:
                        features = out_anchor[4].view(-1, out_anchor[4].shape[-1])
